<template>
  <div class="fixed bottom-6 right-6 z-50 w-80
              p-5 bg-gradient-to-br from-slate-800/90 to-slate-900/90 rounded-2xl backdrop-blur-sm
    ...
    <!-- 装饰性舱窗 -->
    <div class="absolute -right-4 -top-4 w-16 h-16 bg-cyan-500/10 rounded-full blur-md"></div>
    <div class="absolute right-8 top-8 w-3 h-3 bg-cyan-400/30 rounded-full"></div>
    <div class="absolute right-12 top-6 w-2 h-2 bg-cyan-400/20 rounded-full"></div>
    
    <!-- 飞船舱门装饰线 -->
    <div class="absolute left-0 top-0 h-full w-1 bg-gradient-to-b from-cyan-500/0 via-cyan-500/20 to-cyan-500/0"></div>
    
    <div class="flex items-center space-x-3 mb-4">
      <div class="relative">
        <span class="text-2xl">🛸</span>
        <!-- 脉冲指示灯 -->
        <div class="absolute -right-1 -bottom-1 w-2 h-2 bg-cyan-400 rounded-full animate-pulse"></div>
      </div>
      <h3 class="text-lg font-medium text-cyan-50">国际空间站位置</h3>
    </div>
    
    <div v-if="issData" class="space-y-2 relative">
      <!-- 数据显示面板 -->
      <div class="space-y-2 bg-slate-900/50 rounded-lg p-3 border border-cyan-500/20">
        <p class="text-sm text-cyan-100/80 flex justify-between">
      ...
      <!-- 更新时间 -->
      <p class="text-xs text-cyan-300/60 mt-3 text-right font-mono">
        更新时间: {{ new Date(issData.timestamp * 1000).toLocaleString() }}
      </p>
    </div>
    <div v-else class="text-sm text-cyan-100/60 animate-pulse">
      正在建立连接...
    </div>
  </div>
</template>
